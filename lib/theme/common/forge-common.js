(function () {
    ...
    const interceptLogoClick = (url) => {
        const anchor = document.querySelector('#red-ui-header > span > a')
        if (!anchor) return

        // Get the logo image and instance name
        const img = anchor.querySelector('img')
        const instanceName = anchor.textContent.trim()

        // Create a button for the logo
        const logoBtn = document.createElement('button')
        logoBtn.className = 'red-ui-header-button'
        logoBtn.style.cssText = 'padding: 0; border: none; background: none; cursor: pointer; display: inline-flex; align-items: center;'
        logoBtn.title = 'Toggle FlowFuse Panel'
        logoBtn.appendChild(img.cloneNode(true))

        // Create chevron indicator (drawer starts open, so chevron points left)
        chevronElement = document.createElement('span')
        chevronElement.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width: 100%; height: 100%; color: #999;"><path fill-rule="evenodd" d="M12.79 5.23a.75.75 0 01-.02 1.06L8.832 10l3.938 3.71a.75.75 0 11-1.04 1.08l-4.5-4.25a.75.75 0 010-1.08l4.5-4.25a.75.75 0 011.06.02z" clip-rule="evenodd" /></svg>'
        chevronElement.style.cssText = 'margin-left: 6px; display: inline-flex; align-items: center; width: 20px; height: 20px; cursor: pointer;'

        // Create a separator
        const separator = document.createElement('span')
        separator.textContent = '|'
        separator.style.cssText = 'margin: 0 10px; color: #999; opacity: 0.5; display: inline-flex; align-items: center;'

        // Create a span for the instance name (non-clickable)
        const nameSpan = document.createElement('span')
        nameSpan.textContent = instanceName
        nameSpan.style.cssText = 'margin-left: 0; color: inherit; display: inline-flex; align-items: center;'

        // Replace the anchor with the button and name
        const container = anchor.parentNode
        container.style.cssText = 'display: inline-flex; align-items: center; margin-top: 0;'
        ...
        logoBtn.addEventListener('click', toggleDrawer)
        chevronElement.addEventListener('click', toggleDrawer)

        // Request current drawer state now that chevron exists
        if (context.isEmbedded) {
            window.parent.postMessage({
                type: 'request-drawer-state'
            }, '*')
        }
    }
    ...
    function handleMessage (event) {
        ...
        } else if (event.data.type === 'drawer-state') {
            // Update chevron direction based on drawer state
            if (chevronElement) {
                chevronElement.innerHTML = event.data.payload.open ? '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width: 100%; height: 100%; color: #999;"><path fill-rule="evenodd" d="M12.79 5.23a.75.75 0 01-.02 1.06L8.832 10l3.938 3.71a.75.75 0 11-1.04 1.08l-4.5-4.25a.75.75 0 010-1.08l4.5-4.25a.75.75 0 011.06.02z" clip-rule="evenodd" /></svg>' : '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" style="width: 100%; height: 100%; color: #999;"><path fill-rule="evenodd" d="M7.21 14.77a.75.75 0 01.02-1.06L11.168 10 7.23 6.29a.75.75 0 111.04-1.08l4.5 4.25a.75.75 0 010 1.08l-4.5 4.25a.75.75 0 01-1.06-.02z" clip-rule="evenodd" /></svg>'
            }
        }
    }
