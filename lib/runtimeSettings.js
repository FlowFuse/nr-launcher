function getSettingsFile(settings) {
  ...
  if (settings.settings) {
    ...
    if (settings.settings.timeZone) {
      projectSettings.codeEditor = settings.settings.timeZone;
    }
    if (settings.settings.palette?.allowInstall !== undefined) {
      projectSettings.palette.allowInstall =
        settings.settings.palette.allowInstall;
    }
    if (settings.settings.palette?.nodesExcludes !== undefined) {
      projectSettings.palette.nodesExcludes =
        settings.settings.palette.nodesExcludes
          .split(",")
          .map((fn) => fn.trim())
          .filter((fn) => fn.length > 0);
    }
    if (settings.settings.modules?.allowInstall !== undefined) {
      projectSettings.modules.allowInstall =
        settings.settings.modules.allowInstall;
    }
  }
