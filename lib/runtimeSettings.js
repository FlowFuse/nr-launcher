...
function getSettingsFile (settings) {
    ...
    projectId: '${settings.projectID}'
        ...
        if (settings.settings.httpNodeCORS?.enabled) {
            const methodsList = ['GET', 'POST', 'PUT', 'PATCH', 'HEAD', 'DELETE']
            const methods = []
            methodsList.forEach(m => {
                if (settings.settings.httpNodeCORS[m]) {
                    methods.push(m)
                }
            })
            projectSettings.httpNodeCors = `httpNodeCors: { origin: "${settings.settings.httpNodeCORS.origin}", methods: "${methods.join(',')}"},`
        }
    }
    ...
    if (settings.features?.teamNPM) {
        projectSettings.ffSubFlowExport = {
            enabled: true
        }
    }
    ...
    // Load via page.scripts so they run regardless of active theme; branding is dropped for non-FlowFuse themes.
    const requestedTheme = projectSettings.theme
    const themeCommonDir = path.join(__dirname, 'theme', 'common')
    const pageScripts = [path.join(themeCommonDir, 'forge-platform.js')]
    if (!requestedTheme || ['forge', 'forge-light', 'forge-dark'].includes(requestedTheme)) {
        pageScripts.push(path.join(themeCommonDir, 'forge-branding.js'))
    }
    projectSettings.page_scripts = `scripts: ${JSON.stringify(pageScripts)},`
    // launcher values shared by the flowfuse-common plugin settings and the legacy theme settings
    const ffSettings = {
        launcherVersion: settings.launcherVersion,
        projectURL: `${settings.forgeURL}/instance/${settings.projectID}`
    }
    projectSettings.flowfuseCommonSettings = `"flowfuse-common": ${JSON.stringify(ffSettings)},`
    ...
    if (projectSettings.theme) {
        ...
        projectSettings.themeSettings = `"${activeTheme}": ${JSON.stringify(ffSettings)},`
...
module.exports = {
    ...
    ${projectSettings.flowfuseCommonSettings || ''}
    editorTheme: {
        ${projectSettings.theme}
        page: {
            title: '${projectSettings.page_title}',
            ${projectSettings.page_scripts || ''}
    ...
    flowforge: {
        ...
        ${settings.features?.teamNPM ? 'subflowExport: { enabled: true }' : ''}
    },
    runtimeState: {
        enabled: true,
        ui: true
