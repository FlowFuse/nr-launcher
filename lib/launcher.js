                    ...
                    if (CPU_LIMIT) {
                        if (avg.cpu > (CPU_LIMIT * 0.75)) {
                            if (this.cpuAuditLogged === 0) {
                                await this.logAuditEvent('resources.cpu', { interval: (RESOURCE_POLL_INTERVAL * RESOURCE_ALERT_SAMPLES), threshold: (RESOURCE_ALERT_THRESHOLD * 100) })
                    ...
                    if (MEMORY_LIMIT) {
                        if (avg.ps > (MEMORY_LIMIT * 0.75)) {
                            if (this.memoryAuditLogged === 0) {
                                await this.logAuditEvent('resources.memory', { interval: (RESOURCE_POLL_INTERVAL * RESOURCE_ALERT_SAMPLES), threshold: (RESOURCE_ALERT_THRESHOLD * 100) })
