...
class Launcher {
    ...
    async loadSettings () {
        ...
        try {
            ...
            console.error(`Unable to get settings from ${settingsURL} with error ${err.toString()}`)
            return
        }
        ...
        const mainPath = require.main ? require.main.path : '/tmp' // needed as mocha breaks require.main
        nodesDir.push(path.join(mainPath, 'node_modules', '@flowfuse', 'nr-project-nodes').replace(/\\/g, '/'))
        nodesDir.push(path.join(mainPath, '..', '..', '@flowfuse', 'nr-project-nodes').replace(/\\/g, '/'))
        nodesDir.push(path.join(mainPath, 'node_modules', '@flowfuse', 'nr-tables-nodes').replace(/\\/g, '/'))
        nodesDir.push(path.join(mainPath, '..', '..', '@flowfuse', 'nr-tables-nodes').replace(/\\/g, '/'))
        nodesDir.push(path.join(mainPath, 'node_modules', '@flowfuse', 'nr-file-nodes').replace(/\\/g, '/'))
        nodesDir.push(path.join(mainPath, '..', '..', '@flowfuse', 'nr-file-nodes').replace(/\\/g, '/'))
        ...
        if (this.settings.assistant?.enabled !== false) {
            nodesDir.push(path.join(mainPath, 'node_modules', '@flowfuse', 'nr-assistant').replace(/\\/g, '/'))
            nodesDir.push(path.join(mainPath, '..', '..', '@flowfuse', 'nr-assistant').replace(/\\/g, '/'))
        }
        nodesDir.push(path.join(mainPath, 'node_modules', '@flowfuse', 'nr-mqtt-nodes').replace(/\\/g, '/'))
        nodesDir.push(path.join(mainPath, '..', '..', '@flowfuse', 'nr-mqtt-nodes').replace(/\\/g, '/'))
        nodesDir.push(path.join(mainPath, 'node_modules', '@flowfuse', 'nr-subflow-export').replace(/\\/g, '/'))
        nodesDir.push(path.join(mainPath, '..', '..', '@flowfuse', 'nr-subflow-export').replace(/\\/g, '/'))
        nodesDir.push(mainPath)
        ...
        try {
            this.mcp = new MCP({
                nodeRedUserDir: path.join(this.settings.rootDir, this.settings.userDir),
    ...
    async updatePackage () {
        ...
        if (changed) {
            const installingStartTime = Date.now()
            ...
            return new Promise((resolve, reject) => {
                ...
                child.on('close', (code) => {
                    this.timing.installing = Date.now() - installingStartTime
                    if (code === 0) {
                        resolve()
                    } else {
    ...
    // MCP ROUTE: step 6 (hosted)
    // Called By: launcher route in ./admin.js:204
    // Calls To : MCP helper class methods

    /**
     * Get a list of features supported by the MCP server based on the provided endpoints.
     * @param {import('./mcp.js').McpEndpointSpec[]} endpoints - List of MCP endpoints to query for features
     * @returns {Promise<{[x: string]: Object}>} - List of MCP features supported by the servers
     */
    async getMCPFeatures (endpoints) {
        if (!this.mcp) {
            throw new Error('MCP client not initialized')
        }
        this.logBuffer.add({ level: 'system', msg: 'Getting MCP features' })
        return this.mcp.getFeatures(endpoints)
    }

    /**
     * Call a tool on the MCP server.
     * @param {import('./mcp.js').McpEndpointSpec} endpoint - The MCP endpoint to call the tool on
     * @param {string} name - The name of the tool to call
     * @param {object} input - The input to pass to the tool
     * @returns {Promise<object>} - The result of the tool call
     */
    async callMCPTool (endpoint, name, input) {
        if (!this.mcp) {
            throw new Error('MCP client not initialized')
        }
        this.logBuffer.add({ level: 'system', msg: 'Calling MCP tool' })
        return this.mcp.callTool(endpoint, name, input)
    }

    /**
     * Read a resource from the MCP server.
     * @param {import('./mcp.js').McpEndpointSpec} endpoint - The MCP endpoint to read the resource from
     * @param {string} uri - The URI of the resource to read
     * @returns {Promise<object>} - The result of the resource read
     */
    async readMCPResource (endpoint, uri) {
        if (!this.mcp) {
            throw new Error('MCP client not initialized')
        }
        this.logBuffer.add({ level: 'system', msg: 'Reading MCP resource' })
        return this.mcp.readResource(endpoint, uri)
    }

    // #endregion MCP Support

    async start (targetState) {
        const NRstartTime = Date.now()
        ...
        // The user's node_modules may have just been (re)installed, so any
        // previously-resolved MCP SDK could now be stale. Drop the cached
        ...
        const statusPoll = async () => {
            ...
            try {
                ...
                console.error('Failed to poll NR on ', pollUrl, error)
            ...
            if (statusCode >= 200 && statusCode < 500) {
                if (this.state === States.STARTING) {
                    this.timing.NRstartUp = Date.now() - NRstartTime
                    console.info(`START-UP-TIMES: ${JSON.stringify(this.timing)}`)
                }
    ...
    async stop () {
        ...
        // Drop any cached MCP SDK reference: the SDK files may be removed or
        // replaced before the next start, so they must not be reused.
        this.mcp?.resetSdk?.()
    ...
    async reportStateChange (state) {
        // we only need to report definitive states
        const isReportableState = [States.STOPPED, States.RUNNING, States.SAFE, States.CRASHED].includes(state)
        const isPreviouslyReportedState = this._lastReportedState === state

        if (!isReportableState || isPreviouslyReportedState) {
            return
        }

        const url = `${this.options.forgeURL}/api/v1/projects/${this.options.project}/update-state`
        ...
        try {
            ...
            console.error(`Unable to report state change to ${url} with error ${err.toString()}`)
        }
    }
}
