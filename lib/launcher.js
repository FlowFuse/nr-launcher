...
const { hasProperty } = require('../../device-agent/lib/utils')
...
class Launcher {
    ...
    async logAuditEvent (event, body) {
        ...
        return got.default.get.post(this.options.forgeURL + '/logging/' + this.options.project + '/audit', {
            json: data,
            headers: {
                authorization: 'Bearer ' + this.options.token
            }
    ...
    async start (targetState) {
        ...
        const statusPoll = async () => {
            ...
            try {
                ...
                const res = await got.default.get.head(pollUrl, opts)
