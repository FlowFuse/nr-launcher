...
class Launcher {
    ...
    /** @type {Number} */
    get healthPollInterval () {
        // Ensure the health poll interval is at least 1 second
        if (typeof this.settings?.healthPollInterval === 'number' && this.settings.healthPollInterval > 1000) {
            return this.settings.healthPollInterval
        }
        return HEALTH_POLL_INTERVAL_DEFAULT
    }
    ...
    async loadSettings () {
        ...
        this.settings.healthPollInterval = this.options?.healthPollInterval || process.env.FORGE_HEALTH_POLL_INTERVAL
        ...
        const statusPoll = async () => {
            ...
            try {
                const opts = {
                    ...
                    timeout: { request: this.healthPollInterval - 500 },
                    retry: { limit: 0 }
                }
