...
function deriveSessionSecret (credentialSecret) {
    return credentialSecret
        ? crypto.createHmac('sha256', credentialSecret).update('nr-launcher:httpAuth:session').digest('hex')
...
module.exports = {
    ...
    setupAuthRoutes (app) {
        ...
        const crossSiteCookie = (options.baseURL || '').startsWith('https://')
        httpNodeApp.use(session({
            secret: deriveSessionSecret(options.credentialSecret),
            resave: false,
            saveUninitialized: false,
            store: new MemoryStore({
                checkPeriod: 86400000 // prune expired entries every 24h
            }),
            cookie: crossSiteCookie ? { sameSite: 'none', secure: true, partitioned: true } : {}
        }))
