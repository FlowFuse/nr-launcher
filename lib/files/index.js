...
const  filesInterface = (app, settings)  => {
    ...
    app.get(/^\/flowforge\/files\/_\/([\.a-zA-Z0-9\/\-_]*)$/, async (request, reply) => {
        try {
    ...
    app.put(/^\/flowforge\/files\/_\/([\.a-zA-Z0-9\/\-_]*)$/, async (request, reply) => {
    ...
    app.post(/^\/flowforge\/files\/_\/([\.a-zA-Z0-9\/\-_]*)$/, upload.single('file'), async (request, reply) => {
    ...
    app.delete(/^\/flowforge\/files\/_\/([\.a-zA-Z0-9\/\-_]*)$/, async (request, reply) => {
        const fullPath = normalize(join(BASE_PATH, request.params[0]))
