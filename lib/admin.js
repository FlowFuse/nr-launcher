    ...
    constructor (options, launcher) {
        ...
        app.post('/flowforge/command', async (request, response) => {
            if (request.body.cmd === 'stop') {
                ...
                response.send({})
                process.exit(0)
            } else if (request.body.cmd === 'mcp:get-features') {
                // MCP ROUTE: step 5 (hosted)
                // Called By: driver (localfs/k8s/docker)
                // Calls To : ./launcher.js helper fn callMCPTool
                try {
                    const { endpoints } = request.body?.data || {}
                    const features = await launcher.getMCPFeatures(endpoints)
                    response.send(features)
                } catch (error) {
                    response.status(500).send({ err: error.message })
                }
            } else if (request.body.cmd === 'mcp:call-tool') {
                try {
                    const { endpoint, name, input } = request.body?.data || {}
                    const result = await launcher.callMCPTool(endpoint, name, input)
                    response.send(result)
                } catch (error) {
                    response.status(500).send({ err: error.message })
                }
            } else if (request.body.cmd === 'mcp:read-resource') {
                try {
                    const { endpoint, uri } = request.body?.data || {}
                    const result = await launcher.readMCPResource(endpoint, uri)
                    response.send(result)
                } catch (error) {
                    response.status(500).send({ err: error.message })
                }
